...
pub fn build(b: *std.build.Builder) void {
    ...
    const windows_kit_bin_root = b.option([]const u8, "windows_kit_bin_root", "Windows SDK Bin root") orelse "C:/Program Files (x86)/Windows Kits/10/bin/10.0.26100.0";
    ...
    b.installFile("etc/keybinds.txt", b.fmt("{s}/keybinds.txt", .{prefix}));
    ...
    exe.addCSourceFile("src/crashlog.c", &.{""});
    ...
    exe.addCSourceFile("src/disconnect.c", &.{""});
    ...
    const iupLib = switch (arch) {
        .x64 => "external/iup-3.32_Win64_mingw6_lib",
        .x86 => "external/iup-3.32_Win32_mingw6_lib",
    };

    exe.addIncludeDir(b.pathJoin(&.{ iupLib, "include" }));
    exe.addCSourceFile(b.pathJoin(&.{ iupLib, "libiup.a" }), &.{""});

    //    exe.addLibPath("external/iup-3.32_Win64_dll16_lib");
    //    exe.linkSystemLibrary("iupcontrols");
    //    exe.linkSystemLibrary("iupcd");
